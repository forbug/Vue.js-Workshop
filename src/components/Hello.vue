    ...
    <div class="col-lg-12">
      ...
      <ul class='list-group'>
        <li class='list-group-item' v-for="each in todos" v-bind:key="each.label">
          ...
          <button class="btn btn-sm btn-danger pull-right" v-if="!each.editing" v-on:click="trashTodo(each)">
            <i class="fa fa-trash"></i>
          </button>

          <!-- STEP 4 - EDIT METHOD -->
          <button class="btn btn-sm btn-warning pull-right" v-if="!each.editing" v-on:click="editTodo(each)">
            <i class="fa fa-pencil"></i>
          </button>

          <!-- Form -->
          <input type="text" v-model="each.editing" v-if="each.editing">
          <button class="btn btn-sm btn-success pull-right" v-if="each.editing" v-on:click="submitEdit(each)">
            <i class="fa fa-check"></i>
          </button>
          <button class="btn btn-sm btn-dark pull-right" v-if="each.editing" v-on:click="cancelEdit(each)">
      ...
      <!-- NEXT - INTRODUCE COMPONENTS -->
      <!-- LETS ABSTRACT THE NEW FORM INTO A COMPONENT -->
      <!-- 1 - NEW FILE -->
      <!-- 2 - import component -->
      <!-- 3 - setup components:{} -->
      <!-- 4 - insert <TodoForm/> into template -->
      <!-- 5 - but this isn't really wired up yet - introduce props -->
